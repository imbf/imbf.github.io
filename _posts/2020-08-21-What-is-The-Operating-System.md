...
가장 일반적인 다중 처리기 시스템은 각 **피어 CPU 프로세스가 운영체제 기능 및 사용자 프로세스를 포함한 모든 작업을 수행하는 SMP(symmetric multiprocessing)**를 사용한다. 이 모델의 장점은 성능을 크게 저하하지 않으면서 N개의 프로세스를 실행할 수 있다. <u>그러나 CPU가 독립적이기 때문에 하나는 유휴 상태이고 다른 하나는 과부하가 걸려서 비효율적일 수 있다.</u> => 스케줄링을 잘 해야 한다.
...
**NUMA Multi Processor Architecture**은 다음과 같다.
